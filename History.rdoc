...
* Removed vestigial Distro package_map, and use in RHEL, Ubuntu,
  PostgreSQL and Hashdot. Better to include Distro specific package
  variations in the specific components, rather than two places.
* SyncWrap::RunUser now creates a _system_ user/group with default
  home the same as run_dir.
* SyncWrap::JRubyVM default version updated to 1.7.13 (incl all
  example usage.)
* Broadened aws-sdk to >= 1.34.0, < 1.39
* Upgrade default CRubyVM to 2.1.2
