...
* Add optional SyncWrap::Puma#systemd_socket (activation) support
    * Puma#systemd_unit property is deprecated in favor of #systemd_service
...
* Add Puma#puma_restart and #puma_stop (CLI) utility methods
* Don't specify an alternative default port for SyncWrap::Puma (as
  user might rather to do that in config). You'll need to update your
  config if you relied on this default.
* Attempt to have CLI (-l) listings output wrap to (unix) terminal width
* SyncWrap::Iyyov with systemd should also conditionally daemon-reload
* Upgrade SyncWrap::CRubyVM default version to 2.1.10
* Upgrade SyncWrap::JRubyVM default version to 1.7.24
