...
* Simplify existing state handling (i.e. Ubuntu.first_apt?, Users
  :just_created) by introducing SyncWrap::Component#state (via
  Context)
...
* Add CLI --create-image, AmazonEC2 support
* Add GitHelp module for using a git hash as a host or image tag from
  profile.
