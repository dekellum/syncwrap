...
* Update SyncWrap::PostgreSQL for support of version 9.5 as of the
  Beta 2 release. Changes, including postgresql.conf updates,
  remain backward compatible with 9.1+ and SyncWrap 2.3.0+
* Add a check_update option to Distro#dist_install for all
  ...
  default for now, but is used for more complete expression of some
  existing component dependencies (see below). To enable
  it globally, add `options( check_update: true )` to your sync.rb
  file.
...
* Since Debian's `apt-get remove` and Arch's `pacman -R` fail if any
  specified package is not found, test each package for install before
  removing it. The leniency of dist_uninstall is now consistent across
  all distros.
* Some minimal distros like Debian 8 might not include rsync in a base
  image. Add a conditional dist_install of rsync to the
  SyncWrapp::Users component before attempting to rput user home
  dirs. A distro component will needs to be included before Users for
