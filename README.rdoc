...
* Multi-line shell support (often via here-document), command queue,
  ...
  script embedding.  You'll know your provisioning with bash, in all
  its sintantic glory.
...
* Distro differences are factored out into distro-specific components
  with utility methods.

* SyncWrap::AmazonEC2 provider with host profiles: create and
  provision hosts in a single step.

* Independent of (but may be easily intergrated into) rake

* Depends only on ruby 1.9+ stdlib Open3 for interaction with ssh and
  rsync processes. Actually really works on jruby 1.6.8 (--1.9 mode)
  and 1.7+
...
Components implementing SyncWrap::Distro, to expose common utilty
...
* SyncWrap::OpenJDK or SyncWrap::CommericalJDK
...
* MRI Ruby by source compile and/or distro-packages
